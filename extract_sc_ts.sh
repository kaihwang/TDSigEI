...
for s in $(ls -d 5*); do
	...
	for condition in FH Fo Fp HF Ho Hp; do
		...
		cat ${SCRIPT}/${s}_run_order | grep "${condition}" >  ${SCRIPT}/tmp
		...
		for ROI in FFA PPA RH LH; do
			...
			for motormapping in 1 2; do
				...
				r=1
				for run in $(cat ${SCRIPT}/tmp | grep -n "${condition},${motormapping}" | cut -f1 -d:); do
					3dmaskave -mask ${ROI}_indiv_ROI+tlrc -q \
					${s}_FIR_${condition}_errts.nii.gz[${TRrange[$(($run-1))]}] > ${WD}/SC_1Ds/${s}_${ROI}_${condition}_motor${motormapping}_run${r}.1D
					r=$(($r+1))
				done

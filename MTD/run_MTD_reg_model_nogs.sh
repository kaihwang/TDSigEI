	...
	for dset in nusiance FIR; do #FIR 
		...
		for condition in FH Fo Fp HF Ho Hp; do  # Fo Fp HF Ho Hp
			...
			for run in 1 2 3 4; do
				...
				3dTcat -prefix /tmp/${s}/${dset}_Reg_${condition}_errts_run${run}.nii.gz ${WD}/${s}/${s}_${dset}_${condition}_errts_nogs.nii.gz[${TRrange[$(($run-1))]}]
		...
		-rout \
		-tout \
		-bucket /tmp/${s}/${dset}_MTD_BC_stats_nogs \
		-GOFORIT 100 \
		-noFDR

		. /tmp/${s}/${dset}_MTD_BC_stats_nogs.REML_cmd
		
		mv ${dset}_MTD_BC_stats* ${WD}/${s}/
		mkdir ${WD}/${s}/1Ds_nogs
		mv ${dset}_MTDReg*all*1D ${WD}/${s}/1Ds_nogs
		mv ${dset}_BCReg*all*1D ${WD}/${s}/1Ds_nogs

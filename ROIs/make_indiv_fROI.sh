...
for s in $(/bin/ls -d 5*); do #$(ls -d 5*)
	...
	3dmaskdump -mask FFAmasked.nii.gz -quiet FFAmasked.nii.gz | sort -k4 -n -r | head -n 128 | 3dUndump -master FFAmasked.nii.gz -ijk -prefix FFA_indiv_ROI.nii.gz stdin
	...
	3dmaskdump -mask PPAmasked.nii.gz -quiet PPAmasked.nii.gz | sort -k4 -n -r | head -n 128 | 3dUndump -master PPAmasked.nii.gz -ijk -prefix PPA_indiv_ROI.nii.gz stdin
...
# 	3dmaskdump -mask RHmasked.nii.gz -quiet RHmasked.nii.gz | sort -k4 -n -r | head -n 1 | 3dUndump -master RHmasked.nii.gz -srad 8 -ijk -prefix RH_indiv_ROI.nii.gz stdin
...
# 	3dmaskdump -mask LHmasked.nii.gz -quiet LHmasked.nii.gz | sort -k4 -n -r | head -n 1 | 3dUndump -master LHmasked.nii.gz -srad 8 -ijk -prefix LH_indiv_ROI.nii.gz stdin
